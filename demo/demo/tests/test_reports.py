...
def test__reports__subscriptions__overlapping(reports_subscriptions, eps):
    ...
    assert set(SubscriptionsReport(now_-days(1), now_+days(7)+eps).overlapping) == {subs[0], subs[1], subs[2], subs[3]}

    assert set(SubscriptionsReport(now_+days(3), now_+days(7)+eps).overlapping) == {subs[0], subs[1], subs[2], subs[3]}
    assert set(SubscriptionsReport(now_+days(17), now_+days(30)+eps).overlapping) == {subs[0], subs[1], subs[2], subs[3]}
    assert set(SubscriptionsReport(now_+days(17)+timedelta(seconds=1), now_+days(30)+eps).overlapping) == {subs[0], subs[1], subs[2]}
...
def test__reports__subscriptions__new__datetimes(reports_subscriptions, eps):
    ...
    assert list(sorted(SubscriptionsReport(now_, now_+days(7)+eps).get_new_datetimes())) == [now_, now_, now_+days(3), now_+days(7)]
...
def test__reports__subscriptions__ended_or_ending__ages(reports_subscriptions):
    ...
    with freeze_time(now_):
        assert set(SubscriptionsReport(now_, now_+days(32)).get_ended_or_ending_ages()) == {timedelta(days=30), timedelta(days=14)}
...
def test__reports__subscriptions__active__ages(reports_subscriptions):
    ...
    assert sorted(SubscriptionsReport(now_+days(7), now_+days(17)).get_active_ages()) == sorted([timedelta(days=17), timedelta(days=10), timedelta(days=17)])
...
def test__reports__subscriptions__active__plans__quantities(reports_subscriptions, plan, bigger_plan, recharge_plan):
    ...
    assert set(SubscriptionsReport(now_+days(7), now_+days(17)).get_active_plans_and_quantities()) == {(plan, 1), (bigger_plan, 1), (plan, 2)}


def test__reports__subscriptions__active__plans__total(reports_subscriptions, plan, bigger_plan, recharge_plan):
    now_ = reports_subscriptions[0].start

    assert SubscriptionsReport(now_+days(7), now_+days(17)).get_active_plans_total() == Counter({plan: 3, bigger_plan: 1})
...
def test__reports__transactions__payments__query(reports_payments, paddle):
    ...
    assert list(TransactionsReport(provider_codename=paddle.codename, since=now_-days(1), until=now_-timedelta(seconds=1)).payments) == []
    assert list(TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(9)).payments) == [pmts[0]]
    assert list(TransactionsReport(provider_codename=paddle.codename, since=now_+days(1), until=now_+days(10)).payments) == [pmts[1], pmts[4]]
...
def test__reports__transactions__payments__count_by_status(reports_payments, paddle):
    ...
    assert dict(TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(30)).get_payments_count_by_status()) == {
...
def test__reports__transactions__payments__completed__query(reports_payments, paddle):
    ...
    assert list(TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(20)).completed_payments) == [pmts[0], pmts[1], pmts[2], pmts[6]]


def test__reports__transactions__payments__completed__amounts(reports_payments, paddle):
    now_ = reports_payments[0].created

    amounts = TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(30)).get_completed_payments_amounts()
...
def test__reports__transactions__payments__completed__average(reports_payments, paddle):
    ...
    assert TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(30)).get_completed_payments_average() == usd(170)
    assert TransactionsReport(provider_codename=paddle.codename, since=now_-days(2), until=now_-days(1)).get_completed_payments_average() is None


def test__reports__transactions__payments__completed__total(reports_payments, paddle):
    now_ = reports_payments[0].created

    assert TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(30)).get_completed_payments_total() == usd(680)


def test__reports__transactions__payments__incompleted__amounts(reports_payments, paddle):
    now_ = reports_payments[0].created

    amounts = TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(30)).get_incompleted_payments_amounts()
...
def test__reports__transactions__payments__incompleted__total(reports_payments, paddle):
    ...
    TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(30)).get_incompleted_payments_total() == usd(800)


def test__reports__transactions__refunds__query(reports_payments, paddle):
    now_ = reports_payments[0].created
    pmts = reports_payments
    assert list(TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(20)).refunds) == [pmts[-2]]


def test__reports__transactions__refunds__count(reports_payments, paddle):
    now_ = reports_payments[0].created

    assert TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(20)).get_refunds_count() == 1


def test__reports__transactions__refunds__amounts(reports_payments, paddle):
    now_ = reports_payments[0].created

    assert TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(20)).get_refunds_amounts() == [usd(250)]


def test__reports__transactions__refunds__average(reports_payments, paddle):
    now_ = reports_payments[0].created

    assert TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(1)).get_refunds_average() is None
    assert TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(20)).get_refunds_average() == usd(250)


def test__reports__transactions__refunds__total(reports_payments, paddle):
    now_ = reports_payments[0].created

    assert TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(1)).get_refunds_total() == NO_MONEY
    assert TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(20)).get_refunds_total() == usd(250)


def test__reports__transactions__estimated_recurring_charge__by_time(reports_subscriptions, paddle, eps):
    now_ = reports_subscriptions[0].start

    assert TransactionsReport(provider_codename=paddle.codename, since=now_-days(2), until=now_-days(1)).get_estimated_recurring_charge_amounts_by_time() == {}
    assert TransactionsReport(provider_codename=paddle.codename, since=now_-days(1), until=now_+days(2)).get_estimated_recurring_charge_amounts_by_time() == {
        now_: usd(100) * 3,
    }
    assert TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(7)+eps).get_estimated_recurring_charge_amounts_by_time() == {
        now_: usd(100) * 3,
        now_+days(7): usd(200),
    }
    assert TransactionsReport(provider_codename=paddle.codename, since=now_+days(3), until=now_+days(7)+eps).get_estimated_recurring_charge_amounts_by_time() == {
        now_+days(7): usd(200),
    }
    assert TransactionsReport(provider_codename=paddle.codename, since=now_-days(1), until=now_+days(40)).get_estimated_recurring_charge_amounts_by_time() == {
        now_: usd(100) * 3,
        now_+days(7): usd(200),
...
def test__reports__transactions__estimated_recurring_charge__total(reports_subscriptions, paddle, eps):
    ...
    assert TransactionsReport(provider_codename=paddle.codename, since=now_-days(2), until=now_-days(1)).get_estimated_recurring_charge_total() == NO_MONEY
    assert TransactionsReport(provider_codename=paddle.codename, since=now_-days(1), until=now_+days(2)).get_estimated_recurring_charge_total() == usd(300)
    assert TransactionsReport(provider_codename=paddle.codename, since=now_, until=now_+days(7)+eps).get_estimated_recurring_charge_total() == usd(500)
    assert TransactionsReport(provider_codename=paddle.codename, since=now_+days(3), until=now_+days(7)+eps).get_estimated_recurring_charge_total() == usd(200)
    assert TransactionsReport(provider_codename=paddle.codename, since=now_-days(1), until=now_+days(40)).get_estimated_recurring_charge_total() == usd(1000)
