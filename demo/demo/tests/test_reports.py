...
def test__reports__subscriptions__iter_periods(now):
...
def test__reports__subscriptions__overlapping(reports_subscriptions, now, eps):
    subs = reports_subscriptions

    assert set(SubscriptionsReport(now-days(1), now-timedelta(seconds=1)).overlapping) == set()
    assert set(SubscriptionsReport(now-days(1), now).overlapping) == set()
    assert set(SubscriptionsReport(now-days(1), now+eps).overlapping) == {subs[0], subs[2]}
    assert set(SubscriptionsReport(now-days(1), now+days(2)).overlapping) == {subs[0], subs[2]}
    assert set(SubscriptionsReport(now-days(1), now+days(3)+eps).overlapping) == {subs[0], subs[2], subs[3]}
    assert set(SubscriptionsReport(now-days(1), now+days(4)).overlapping) == {subs[0], subs[2], subs[3]}
    assert set(SubscriptionsReport(now-days(1), now+days(7)+eps).overlapping) == {subs[0], subs[1], subs[2], subs[3]}

    assert set(SubscriptionsReport(now+days(3), now+days(7)+eps).overlapping) == {subs[0], subs[1], subs[2], subs[3]}
    assert set(SubscriptionsReport(now+days(17), now+days(30)+eps).overlapping) == {subs[0], subs[1], subs[2], subs[3]}
    assert set(SubscriptionsReport(now+days(17)+timedelta(seconds=1), now+days(30)+eps).overlapping) == {subs[0], subs[1], subs[2]}
...
def test__reports__subscriptions__new(reports_subscriptions, now, eps):
    subs = reports_subscriptions

    assert set(SubscriptionsReport(now-days(1), now-timedelta(seconds=1)).new) == set()
    assert set(SubscriptionsReport(now-days(1), now).new) == set()
    assert set(SubscriptionsReport(now-days(1), now+eps).new) == {subs[0], subs[2]}
    assert set(SubscriptionsReport(now, now+days(7)+eps).new) == {subs[0], subs[1], subs[2], subs[3]}
    assert set(SubscriptionsReport(now+days(1), now+days(2)).new) == set()
    assert set(SubscriptionsReport(now+days(1), now+days(3)+eps).new) == {subs[3]}
...
def test__reports__subscriptions__new__count(reports_subscriptions, now, eps):
    ...
    assert SubscriptionsReport(now, now+days(3)+eps).get_new_count() == 3
    assert SubscriptionsReport(now+days(2), now+days(3)+eps).get_new_count() == 1
    assert SubscriptionsReport(now+days(3), now+days(6)).get_new_count() == 1
    assert SubscriptionsReport(now+days(3), now+days(7)+eps).get_new_count() == 2
...
def test__reports__subscriptions__excluding_end(reports_subscriptions, now, eps):

    assert SubscriptionsReport(now+days(3), now+days(7)).get_new_count() == 1
    assert SubscriptionsReport(now+days(3), now+days(7)+eps).get_new_count() == 2


def test__reports__subscriptions__new__datetimes(reports_subscriptions, now, eps):

    assert set(SubscriptionsReport(now-days(2), now-days(1)).get_new_datetimes()) == set()
    assert set(SubscriptionsReport(now+days(1), now+days(3)+eps).get_new_datetimes()) == {now+days(3)}
    assert list(sorted(SubscriptionsReport(now, now+days(7)+eps).get_new_datetimes())) == [now, now, now+days(3), now+days(7)]


def test__reports__subscriptions__ended_or_ending__query(reports_subscriptions, now):
    subs = reports_subscriptions
...
def test__reports__subscriptions__ended_or_ending__count(reports_subscriptions, now):
...
def test__reports__subscriptions__ended_or_ending__datetimes(reports_subscriptions, now):
    subs = reports_subscriptions
...
def test__reports__subscriptions__ended_or_ending__ages(reports_subscriptions, now):
    with freeze_time(now):
        assert set(SubscriptionsReport(now, now+days(32)).get_ended_or_ending_ages()) == {timedelta(days=30), timedelta(days=14)}


def test__reports__subscriptions__active__query(reports_subscriptions, now):
    subs = reports_subscriptions
...
def test__reports__subscriptions__active__count(reports_subscriptions, now, eps):
    assert SubscriptionsReport(now, now+days(17)).get_active_count() == 3


def test__reports__subscriptions__active__users_count(reports_subscriptions, now):
...
def test__reports__subscriptions__active__ages(reports_subscriptions, now):
...
def test__reports__subscriptions__active__plans__quantities(reports_subscriptions, now, plan, bigger_plan, recharge_plan):
    assert set(SubscriptionsReport(now+days(7), now+days(17)).get_active_plans_and_quantities()) == {(plan, 1), (bigger_plan, 1), (plan, 2)}


def test__reports__subscriptions__active__plans__total(reports_subscriptions, now, plan, bigger_plan, recharge_plan):
    assert SubscriptionsReport(now+days(7), now+days(17)).get_active_plans_total() == Counter({plan: 3, bigger_plan: 1})


def test__reports__transactions__payments__query(reports_payments, paddle, now):
...
def test__reports__transactions__payments__count_by_status(reports_payments, paddle, now):
...
def test__reports__transactions__payments__completed__query(reports_payments, paddle, now):
    pmts = reports_payments
...
def test__reports__transactions__payments__completed__amounts(reports_payments, paddle, now):
...
def test__reports__transactions__payments__completed__average(reports_payments, paddle, now):
...
def test__reports__transactions__payments__completed__total(reports_payments, paddle, now):
...
def test__reports__transactions__payments__incompleted__amounts(reports_payments, paddle, now):
...
def test__reports__transactions__refunds__total(reports_payments, paddle, now):
...
def test__reports__transactions__estimated_recurring_charge__by_time(reports_subscriptions, paddle, now, eps):
    ...
    assert TransactionsReport(provider_codename=paddle.codename, since=now, until=now+days(7)+eps).get_estimated_recurring_charge_amounts_by_time() == {
        now: usd(100) * 3,
        now+days(7): usd(200),
    }
    assert TransactionsReport(provider_codename=paddle.codename, since=now+days(3), until=now+days(7)+eps).get_estimated_recurring_charge_amounts_by_time() == {
        now+days(7): usd(200),
    }
...
def test__reports__transactions__estimated_recurring_charge__total(reports_subscriptions, paddle, now, eps):
    ...
    assert TransactionsReport(provider_codename=paddle.codename, since=now, until=now+days(7)+eps).get_estimated_recurring_charge_total() == usd(500)
    assert TransactionsReport(provider_codename=paddle.codename, since=now+days(3), until=now+days(7)+eps).get_estimated_recurring_charge_total() == usd(200)
