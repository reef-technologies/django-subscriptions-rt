...
class AppleInAppProvider(Provider):
    ...
    def webhook(self, request: Request, payload: dict) -> Response:
        ...
        for request_class, handler in handlers.items():
            ...
            logger.error(
                "Failed matching the payload to any registered request:\n%s.", "\n\n".join(validation_error_messages)
            )
    ...
    def _get_active_transaction(self, transaction_id: str, original_transaction_id: str) -> SubscriptionPayment:
        ...
        try:
            ...
            # This is left as a countermeasure in case the deduplication fails or
            # the code is still "not good enough" and generates duplicates.
            # It allows us to read a warning from sentry instead of rushing another fix.
            logger.warning(
                'Multiple payments found when fetching active transaction id "%s". '
    ...
    def _handle_single_receipt_info(
        self, user: User, receipt_info: AppleLatestReceiptInfo
    ...
    def _handle_subscription_change(self, notification: AppStoreNotification) -> None:
        ...
        if notification.subtype == AppStoreNotificationTypeV2Subtype.DOWNGRADE:
            logger.info(
                'Downgrade requested for original transaction id "%s" to product "%s". '
    ...
    def _handle_refund(self, notification: AppStoreNotification) -> None:
        ...
        try:
            ...
            logger.warning(
                'Refund called on unknown transaction id "%s". Searching for latest payment for given '
    ...
    def _handle_app_store(self, _request: Request, payload: AppleAppStoreNotification) -> Response:
        ...
        if handler is None:
            logger.info(
                "Received apple notification %s (%s) and ignored it. Payload: %s",
