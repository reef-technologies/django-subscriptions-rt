...
def make_user_plans_and_subscriptions(
    apps: Apps, num_plans: int = 1, num_subscriptions_per_plan: int = 1
    ...
    result_subscriptions = [
        subscription_model.objects.create(
            ...
            plan=plan,
            quantity=1,
            start=datetime.datetime(2022, 3, 1, tzinfo=datetime.UTC),
            end=datetime.datetime(2022, 3, 2, tzinfo=datetime.UTC),
        )
...
def test__deduplication_migration__check_migration_deduplication(apple_in_app, days):
    ...
    def pre_migration_run(pre_apps: Apps) -> None:
        ...
        kwargs = {"provider_codename": apple_in_app.codename, "user": user, "plan": plans[0], **BASIC_PAYMENT_KWARGS}

        def make_payment_with_subscription(id_: str, index: int) -> None:
            pre_model.objects.create(
                uid=str(uuid.uuid4()), provider_transaction_id=id_, subscription=subscriptions[index], **kwargs
            )
...
def test__deduplication_migration__fail__same_transaction_different_plan(apple_in_app, days):
    def pre_migration_run(pre_apps) -> None:
        ...
        kwargs = {"provider_codename": apple_in_app.codename, "user": user, **BASIC_PAYMENT_KWARGS}
...
def test__deduplication_migration__fail__multiple_reused_subscriptions(apple_in_app, days):
    def pre_migration_run(pre_apps) -> None:
        ...
        pre_model.objects.create(
            uid=str(uuid.uuid4()), provider_transaction_id="transaction_id_1", subscription=subscriptions[0], **kwargs
        )
        pre_model.objects.create(
            uid=str(uuid.uuid4()), provider_transaction_id="transaction_id_1", subscription=subscriptions[0], **kwargs
        )
        pre_model.objects.create(
            uid=str(uuid.uuid4()), provider_transaction_id="transaction_id_1", subscription=subscriptions[1], **kwargs
        )
        pre_model.objects.create(
            uid=str(uuid.uuid4()), provider_transaction_id="transaction_id_1", subscription=subscriptions[1], **kwargs
        )
